...
const images = [
  {
    src: '/assets/hero_slider_1.png',
    alt: 'Advanced Orthopedic Trauma Center'
  },
  {
    src: '/assets/hero_slider_2.png',
    alt: 'Spine Surgery Operation Theater'
  },
  {
    ...
    <div className="relative w-full h-[300px] sm:h-[400px] md:h-[500px] rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-slate-200 dark:bg-slate-800">
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
        {images.map((_, idx) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${idx === current ? 'w-8 bg-primary' : 'w-2 bg-white/50'
              }`}
          />
        ))}
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center p-8 bg-slate-900/20 backdrop-blur-sm rounded-2xl border border-white/10 z-20">
          <h3 className="text-2xl font-bold text-white mb-2">Hemant Trauma and Sport Injury Centre</h3>
